...
set_time_limit(30);		// Лимит времени выполнения операции (в секундах)
...
$proxy = '10.60.180.250:3128';													// Настройки прокси, если используется (IP:port, например 10.20.30.40:1234
...
if ((isset($TaxcomSessionToken)) AND ($TaxcomSessionToken != "") AND ($TaxcomSessionToken != NULL))
{
	...
	if ((isset($department_id)) AND ($department_id != "") AND ($department_id != NULL))
	{
		...
		$id_array = array_column($OutletList_json['records'],'id');	// Получаем массив из нужных id от касс нашего подразделения
		$id_array_count = count($id_array);	// Получаем общее количество наших касс
		
		// Если нужные кассы имеются
		if ($id_array_count > 0)
		{

			// Запускаем цикл операций для каждой кассы
			for ($i=0; $i<$id_array_count; $i++)
			{
				$id_kassa = $id_array[$i];
				...
				if ((isset($id_kassa)) AND ($id_kassa != "") AND ($id_kassa != NULL))
				{
					...
					$url_KKTList = 'https://api-lk-ofd.taxcom.ru/API/v2/KKTList?id=' . $id_kassa . '';
					...
					if (isset($KKTList_json['records'][0]['kktRegNumber']))
					{
						$kktRegNumber = $KKTList_json['records'][0]['kktRegNumber'];
					}
					else
					{
						$kktRegNumber = "";
					...
					$url_FnHistory = 'https://api-lk-ofd.taxcom.ru/API/v2/FnHistory?id=' . $id_kassa . '&kktRegNumber=' . $kktRegNumber . '&np=active';
